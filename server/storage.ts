...
export class MemStorage implements IStorage {
  ...
  async createPoem(poem: InsertPoem, authorId: number): Promise<Poem> {
    ...
    const newPoem: Poem = {
      ...
      isVideo: poem.isVideo ?? null, // Ensure isVideo is either boolean or null
      videoUrl: poem.videoUrl ?? null, // Ensure videoUrl is either string or null
    };
  ...
  async getPoemLikeCount(poemId: number): Promise<number> {
    ...
    let count = 0;
    for (const [key, entry] of Array.from(this.userPoems.entries())) {
  ...
  async createBook(book: InsertBook, uploadedById?: number): Promise<Book> {
    ...
    const newBook: Book = {
      ...
      description: book.description ?? null, // Ensure description is either string or null
      coverImage: book.coverImage ?? null, // Ensure coverImage is either string or null
      genre: book.genre ?? null, // Ensure genre is either string or null
    };
  ...
  async createEvent(event: InsertEvent): Promise<Event> {
    ...
    const newEvent: Event = {
      ...event,
      id,
      description: event.description ?? null, // Ensure description is either string or null
      ticketPrice: event.ticketPrice ?? null, // Ensure ticketPrice is either number or null
      organizer: event.organizer ?? null, // Ensure organizer is either string or null
      streamUrl: event.streamUrl ?? null, // Ensure streamUrl is either string or null
      createdById: event.createdById ?? null, // Ensure createdById is either number or null
    };
  ...
  async createChatRoom(chatRoom: InsertChatRoom, createdById: number): Promise<ChatRoom> {
    ...
    const newChatRoom: ChatRoom = {
      ...
      id,
      createdById,
      description: chatRoom.description ?? null, // Ensure description is explicitly null if undefined
      isPrivate: chatRoom.isPrivate ?? null, // Ensure isPrivate is explicitly null if undefined
    };
  ...
  async createPayment(payment: InsertPayment): Promise<Payment> {
    ...
    const newPayment: Payment = {
      ...
      refundReason: null,
      currency: payment.currency ?? null, // Ensure currency is explicitly null if undefined
    };
  ...
  private seedData() {
    ...
    this.events.set(1, {
      ...
      isFree: true,
      category: null, // Add category property
    });
    this.events.set(2, {
      id: 2,
