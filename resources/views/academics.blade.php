...
<div class="min-h-screen bg-stone-50">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            ...
            <div class="bg-white border border-gray-200 hover:border-gray-300 transition-colors">
                ...
                <div class="p-4 sm:p-6">
                    ...
                    <p class="text-gray-600 text-xs sm:text-sm mb-3 font-light">{{ $resource->description ? Str::limit($resource->description, 50) : 'No description' }}</p>
                    <div class="flex items-center justify-between mb-3 pb-3 border-b border-gray-200">
                        <span class="text-xs text-gray-500">{{ $resource->subject ?? 'General' }}</span>
                        <span class="text-xs text-gray-500">{{ ucfirst(str_replace('_', ' ', $resource->type ?? 'Document')) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">{{ ucfirst(str_replace('_', ' ', $resource->type ?? 'Document')) }}</span>
                        <a href="{{ route('academics.show', $resource) }}" class="text-xs text-gray-700 hover:text-gray-900 font-normal uppercase tracking-wide">Read →</a>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="mb-10 sm:mb-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            ...
            @endphp
            <a href="/academics?subject={{ urlencode($subject) }}" class="bg-white border border-gray-200 p-4 sm:p-6 text-center hover:border-gray-300 transition-colors cursor-pointer block">
                ...
                <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ $subjectCount }} {{ $subjectCount === 1 ? 'paper' : 'papers' }}</p>
            </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="mb-10 sm:mb-12">
            ...
            <div class="bg-white border border-gray-200 p-4 sm:p-6 hover:border-gray-300 transition-colors">
                <div class="flex items-start space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-gray-600 text-xs sm:text-sm mb-2 font-light">{{ $paper->subject ?? 'General' }} • {{ ucfirst(str_replace('_', ' ', $paper->type ?? 'Document')) }}</p>
                        <p class="text-xs text-gray-500 mb-3">Published {{ $paper->created_at->diffForHumans() }}</p>
                        @if($paper->description)
                        <p class="text-xs text-gray-600 mb-3 line-clamp-2">{{ Str::limit($paper->description, 100) }}</p>
                        @endif
                        <div class="flex items-center space-x-4">
                            <a href="{{ route('academics.show', $paper) }}" class="text-xs text-gray-700 hover:text-gray-900 font-normal uppercase tracking-wide">Read Paper →</a>
                            @if($paper->resourceUrl)
                            <a href="{{ $paper->resourceUrl }}" target="_blank" class="text-xs text-gray-500 hover:text-gray-700 font-normal">Download PDF</a>
                            @else
                            <a href="{{ route('academics.download', $paper) }}" class="text-xs text-gray-500 hover:text-gray-700 font-normal">Download PDF</a>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <div class="text-center py-16 sm:py-20">
            <div class="max-w-md mx-auto">
                ...
                <p class="text-sm text-gray-500 mb-6">Check back soon for educational resources and research papers.</p>
            </div>
        </div>
        @endif
    </div>
